...
async def load_metrics_wrapper(algorithm_name: str) -> Dict[str, float]:
    ...
    if not validate_algorithm_name(algorithm_name):  # Corrigé le nom de variable
        raise ValueError(f"Nom d'algorithme invalide : {algorithm_name}")
    
    try:
        result = await asyncio.to_thread(mock_load_algorithm_results, algorithm_name)  # Corrigé le nom de variable
...
async def compare_algorithms(
    ...
    try:
        ...
        logger.error(f"Erreur d'export: {str(e)}")
        raise HTTPException(500, detail="Échec de l'export des résultats")
