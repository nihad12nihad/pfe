...
class SVMModel:
    ...
    def train(self, data_path: str, target_column: str, test_size: float = 0.2):
        ...
        try:
            ...
            logging.error(f"Erreur d'entraînement SVM : {e}")
            return {"error": str(e), "status": "failed"}

    def save_results(self, output_path: str = "app/data/resultats/svm_results.json"):
        ...
        try:
            ...
            logging.error(f"Erreur sauvegarde résultats SVM : {e}")
            raise


def run(X: pd.DataFrame, y: pd.Series, **kwargs) -> Dict[str, Any]:
    """Fonction d'exécution autonome pour usage modulaire."""
    try:
